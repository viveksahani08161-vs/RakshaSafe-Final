...
export function HistoryTimeline({ entries }: { entries: HistoryEntry[] }) {
  return (
    <ol className="relative space-y-5 border-l-2 border-gold-200 pl-5">
      {entries.map((entry) => (
        <li key={entry.id} className="relative">
          <span
            aria-hidden
            className="absolute -left-[27px] top-1 size-3 rounded-full bg-gold-500 ring-4 ring-gold-100"
          />
